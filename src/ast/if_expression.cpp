...
std::unique_ptr<ast::Node> ast::IfExpression::clone() const
{
	...
	if (condition)
	{
		...
		clone->set_condition(std::unique_ptr<Expression>(
			static_cast<Expression *>(cond_clone.release())));
	}

	if (consequence)
	{
		auto stmt = consequence->clone();
		clone->set_consequence(std::unique_ptr<BlockStatement>(
			static_cast<BlockStatement *>(stmt.release())));
	}

	if (alternative)
	{
		auto stmt = alternative->clone();
		clone->set_alternative(std::unique_ptr<BlockStatement>(
			static_cast<BlockStatement *>(stmt.release())));
	}
